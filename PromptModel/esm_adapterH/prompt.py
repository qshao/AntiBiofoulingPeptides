...
class PrefixTuning(nn.Module):
  ...
  Args:
    ...
    input_seq_len (int, optional): The length of the input sequence.
      Defaults to 70.
    prompt_layer_index (int, optional): The index of the layer in the backbone
  ...
  def __init__(self,
              ...
              prompt_layer_index: int = 0,
              device="cpu"):
    super(PrefixTuning, self).__init__()
    self.prompt_len = prompt_len
    self.input_seq_len = input_seq_len
    self.prompt_layer_index = prompt_layer_index
    self.device = device
    ...
    if prompt_layer_index > num_backbone_layers - 1:
      raise ValueError(f"prompt_layer_index {prompt_layer_index} "
    ...
    if prompt_len is not None:
      ...
      self.prompt_layer_dict[f"{prompt_layer_index}"] = (
        from_sample_of_embeddings(token_embed_table)([prompt_len, embed_size]))
  ...
  def forward(self,
    ...
    if (self.prompt_len == 0) or (f"{layer_idx}" not in self.prompt_layer_dict):
      return input_embedding

    # [B,T,E] => [B,prompt+T,E]
    prompted_embedding = self.expand_to_batch(
      self.prompt_layer_dict[f"{layer_idx}"],
      input_embedding)
