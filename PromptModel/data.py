...
def load_GO_seq_annot(file_seq, file_annot):
    ...
    return prot2seq, goterms, gonames, counts
...
# creat the nonfouling dataset - QShao 2-2-2023
...
def prepare_dataloaders_nonfouling(configs):
    train_df = pd.read_csv(configs.train_settings.data_path)
    valid_df = pd.read_csv(configs.valid_settings.data_path)
    test_df = pd.read_csv(configs.test_settings.data_path)

    train_dataset = NfDataset(train_df, configs=configs)
    valid_dataset = NfDataset(valid_df, configs=configs)
    test_dataset  = NfDataset(test_df, configs=configs)
...
if __name__ == '__main__':
    ...
    dataloaders_dict = prepare_dataloaders_nonfouling(configs_file)
    max_position_value = []
    amino_acid = []
